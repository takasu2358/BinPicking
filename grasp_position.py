...
#コンフィグファイルの読み込み
config_path = data_folder_path / "cfg" /"config_file.yaml"
...
class Skeletonize():
    ...
    def skeletonize_region_list(self, region_list, check_result):
        ...
        if len(region_list) - check_count != len(skel_list):
            print(len(check_result))
            print(len(region_list))
            print(check_count)
            print(len(skel_list))
            raise ValueError("Error: skeletonize is failed!")
...
class Sort():
    ...
    #                     continue
    #                 new_skel_list.append(next_poi)
    #                 print("third_next_poi = ", third_next_poi)
    #                 skel.remove(next_poi)
...
class ConnectLine():
    ...
    def __main__(self, sorted_skel_list, end_point_list, correct_line, correct_line_zlist, interpolate_line, correct_connection_index, skel_skip_index, skip_index):
        ...
        if not count_list == []:
            LC_skel_list, z_list, end_point_list, interpolate, correct_connection_index2 = self.connect_line(sorted_skel_list, end_point_list, count_list, switch_list, dis_data, [], [])
...
if __name__ == "__main__":
    ...
    input_image, matrix_image, pcd_matrix, MaxDepth = ply2depth_csv.main(filepath)
